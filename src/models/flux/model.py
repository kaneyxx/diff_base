...
class FluxModel(BaseDiffusionModel):
    """Complete Flux model with Transformer, VAE, and text encoders."""
    ...
    def _build_model(self) -> None:
        ...
        self.transformer = FluxTransformer(transformer_config)

        # VAE
        vae_config = self.config.get("vae", {})
        self.vae = FluxVAE(vae_config)

        # Text encoders
        text_config = self.config.get("text_encoder", {})
        self.text_encoders = FluxTextEncoders(text_config)
        ...
        self.use_guidance = self.config.get("guidance_embeds", True)
    ...
    def forward(
        ...
        **kwargs,
        ...
        Args:
            latents: Latent tensor [B, seq_len, channels].
    ...
    def _load_diffusers_checkpoint(self, path: Path) -> None:
        ...
        """
        logger.info(f"Loading Flux from diffusers format: {path}")
    ...
    def prepare_latents(
        ...
        """
        # Flux latent dimensions
        latent_channels = self.vae.config.get("latent_channels", 16)
