#!/usr/bin/env bash
# Run a LoRA-rank × GPU-count sweep on the reservation node.
#
# rank=8 (light) vs rank=64 (compute-bound) reveals when multi-GPU scaling
# kicks in vs. when single-GPU is enough. Each rank submits 3 jobs (1/2/4 GPU),
# so a 4-rank sweep = 12 jobs total. Sequential between ranks (we wait for one
# rank's 3 jobs to complete before the next) so the reservation isn't oversubscribed.
#
# Usage:
